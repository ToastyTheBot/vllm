...
def test_env_on_off_forward_equivalence_fp8_and_dequantized(
    ...
    # The stock arm goes through the same n-gram-id op as the mmap arm and so
    # needs a forward context too; without one this raised before either arm
    # was compared, which is what made this equivalence check dead code.
    stock_ctx = SimpleNamespace(
        no_compile_layers={stock.layer_name: SimpleNamespace(ple_embedding=stock)}
    )
    with forward_context.override_forward_context(stock_ctx):
...
def test_default_off_forward_never_calls_the_mmap_gather_op(
    ...
    # forward() reaches qwen4_exp_compute_ple_ngram_ids, which resolves the
    # layer out of the forward context; without one it raises before any
    # assertion here can run.
    # The op resolves the PLE *layer* and then its .ple_embedding, so the
    # context must hold a layer-shaped object, not the embedding itself.
    ctx = SimpleNamespace(
        no_compile_layers={
            "model.layers.1.ple": SimpleNamespace(ple_embedding=module)
        }
    )
    with forward_context.override_forward_context(ctx):
...
def _stock_weight_param(
    data: torch.Tensor, *, org_vocab_start_index: int
) -> torch.nn.Parameter:
    """A weight parameter carrying the ``weight_loader`` the stock path calls.

    ``PLEVocabParallelEmbedding`` attaches a ``weight_loader`` to its weight,
    and ple_layer.py invokes it as
    ``embedding.weight.weight_loader(param, loaded_weight, checkpoint_start=N)``
    where N is the shard's first row in *global* vocab coordinates. A bare
    ``nn.Parameter`` has no such attribute, so a double without it does not
    exercise the contract -- it raises AttributeError before reaching any
    assertion.
    """
    ...
    def weight_loader(
        ...
        *,
        ...
        # Intersect the shard's global row range with this rank's slice and
        # copy only the overlap, which is what the stock loader does.
        rank_start = org_vocab_start_index
        rank_end = rank_start + target.shape[0]
        shard_start = checkpoint_start
        shard_end = shard_start + loaded_weight.shape[0]
        lo = max(rank_start, shard_start)
        hi = min(rank_end, shard_end)
        if lo >= hi:
            return
        with torch.no_grad():
            target[lo - rank_start : hi - rank_start] = loaded_weight[
                lo - shard_start : hi - shard_start
            ]
...
def test_default_off_load_weights_matches_the_stock_contract() -> None:
    ...
    embedding = SimpleNamespace(
        org_vocab_size=8,
        embedding_dim=2,
        weight=_stock_weight_param(
            torch.full((4, 2), -1.0), org_vocab_start_index=2
        ),
