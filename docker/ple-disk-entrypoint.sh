...
#
# The KV cache dtype is left at the bf16 default so the image serves any
# checkpoint, but fp8 IS supported on this branch and is what was benchmarked:
# pass --kv-cache-dtype fp8_e4m3 for ~1.9x the KV pool at parity accuracy.
# (An earlier version of this comment claimed the QSA kernel refuses anything
# but bf16. That was true before 2edea6e10c and is now false -- the error
# string it quoted no longer exists in the tree.)
