...
def _qwen4_exp_ple_mmap_forward(
    ...
    ple_embedding_module = getattr(layer, "ple_embedding", None)
    # Upstream renamed the hashing helper `_hash_ngram_ids` -> `compute_ngram_ids`
    # when it split ID generation into its own op. Accept either name so this
    # module works against both, and so the test doubles keep resolving.
    hash_fn = None
    for attr in ("compute_ngram_ids", "_hash_ngram_ids"):
        hash_fn = getattr(ple_embedding_module, attr, None)
        if hash_fn is not None:
            break
    if ple_embedding_module is None or hash_fn is None:
    ...
    ngram_ids = hash_fn(input_ids, query_start_loc, ngram_context)
