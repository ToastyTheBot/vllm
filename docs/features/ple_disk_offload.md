...
(Verbatim from one run; see `ple-disk/final_metrics.txt` in the `investigations`
working directory, which lives outside this repo. Load time
...
(`ple-disk/startup/` in the out-of-repo `investigations` working directory). Worth
stating explicitly because an earlier
